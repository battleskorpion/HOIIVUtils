...
//      () => {
//        FocusTreeManager.clear() // Clear before each test
//        val focusTree = new FocusTree(testFile)
...
//        val retrievedFocusTree = FocusTreeManager.get(testFile)
...
//      assertTrue(FocusTreeManager.focusTrees.size >= createdTrees.size,
