...
class SimplifiedChineseLocalizationService(locFileService: LocalizationFileService) extends BaseLocalizationService(locFileService) with LazyLogging {
  ...
  override def languageId: UIO[String] = ZIO.succeed { "simp_chinese" }

  override def toString: String = s"${getClass.getName}{" + "localizations=" + localizationCollection + "}"
}

object SimplifiedChineseLocalizationService {
  val live: URLayer[LocalizationFileService, LocalizationService] =
    ZLayer.fromFunction(SimplifiedChineseLocalizationService(_))
}
