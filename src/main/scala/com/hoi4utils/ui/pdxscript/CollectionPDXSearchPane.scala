...
class CollectionPDXSearchPane[T <: PDXScript[?]](private val pdxScript: CollectionPDX[T]) extends AnchorPane {
  ...
  private def drawEditor(rootVBox: VBox): Unit = {
    ...
    pdxScript match {
      case effect: CollectionPDX[Effect] =>
        ...
        effectListView.setCellFactory((listView: ListView[Effect]) => new ListCell[Effect]() {
  ...
  private def applyFilter(categoryCombo: ComboBox[String], filteredEffects: FilteredList[Effect]): Unit = {
    val searchText = if (searchField.getText == null) ""
    ...
    filteredEffects.setPredicate((effectItem: Effect) => {
      def foo(effectItem: Effect): Boolean = {
        // Category filter
        if (!("All" == selectedCategory) && !effectItem.effectCategory.equalsIgnoreCase(selectedCategory)) return false
        // Search filter
        if (searchText.isEmpty) return true // No search text => all pass
  ...
  private def applyDebugBorders(parent: Parent): Unit = {
    for (node <- parent.getChildrenUnmodifiable.toArray) {
      node match
