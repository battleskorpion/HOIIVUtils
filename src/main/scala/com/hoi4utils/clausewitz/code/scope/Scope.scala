...
object Scope {
  ...
  def of(effect: Effect): Scope = {
    ...
    val effect_identifier = effect.identifier
    val s = getClone(effect_identifier)
    if (s == null) null
    else s
  }

  private def getClone(name: String): Scope = {
    var clone: Scope = null
    try {
      val scope = scopes.get(name)
      if (scope == null) return null
      clone = scope.clone.asInstanceOf[Scope]
    } catch {
  ...
  try new Scope("all_unit_leader", ScopeType.country, ScopeType.leader, ScopeCategory.TRIGGER)
...
class Scope(final val name: String, final private val withinScopeAllowed: Set[ScopeType],
            final private val targetScopeType: ScopeType, final val scopeCategory: ScopeCategory,
            private var withinScope: Scope) extends Cloneable {
  final val name: String = null
  final private val withinScopeAllowed: Set[ScopeType] = null // 'from' (in the context of)

  //	// usually 'country' or 'any'
  final private val targetScopeType: ScopeType = null // 'to'   (targeting)

  final val scopeCategory: ScopeCategory = null
  //	private List<HOI4Script> content;
  private var withinScope: Scope = _
  ...
  override protected def clone: AnyRef = {
    val c = super.clone.asInstanceOf[Scope]
    c
  }

  override def toString: String = name

  def targetScopeType: ScopeType = targetScopeType
