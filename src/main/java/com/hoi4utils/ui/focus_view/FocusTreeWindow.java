		...
		if (HOIIVUtils.getBoolean("dev.mode")) {
			JOptionPane.showMessageDialog(null,
					"dev @end of initialize() [FocusTreeWindow] - loaded focuses: " + focusTree.focuses().size()
							...
							+ "\n" + "draw focus tree: " + HOIIVUtils.getBoolean("draw.focus.tree.enabled"));
		}
	}
	...
	public void handleFocusTreeViewMousePressed(MouseEvent e) {
		if (e.isPrimaryButtonDown()) {
			...
			draggedFocus = CollectionConverters.asJavaCollection(focusTree.focuses()).stream()
					...
					.findFirst()
					.orElse(null);
			if (HOIIVUtils.getBoolean("dev.mode") && draggedFocus != null) {
			...
			addFocusItem.setOnAction(event -> {
				if (HOIIVUtils.getBoolean("dev.mode")) System.out.println("Adding focus via context menu");
			...
			newFocusTreeItem.setOnAction(event -> {
				if (HOIIVUtils.getBoolean("dev.mode")) System.out.println("Creating new focus tree via context menu");
	...
	public void handleFocusTreeViewMouseReleased(MouseEvent e) {
		...
		if (marqueeStartPoint != null && marqueeEndPoint != null) {
			...
			for (Focus focus : selectedFocuses) {
				...
				if (HOIIVUtils.getBoolean("dev.mode")) System.out.println("Marquee selected focus: " + focus);
			}
		...
		if (event.getButton() == MouseButton.PRIMARY) {
			...
			if (!selectedFocuses.isEmpty()) {
				...
				setRelativeFocusItem.setOnAction(e -> {
					// Replace this with your desired action
					if (HOIIVUtils.getBoolean("dev.mode")) System.out.println("Set relative focus for selected focuses");
				});
