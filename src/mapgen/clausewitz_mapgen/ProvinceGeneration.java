...
public class ProvinceGeneration {
    ...
    public static void provinceGeneration() {
        ...
        /*
         * initialize mapping of seeds to states (regions for purposes of province
         * generation)
         */
        ...
        // try {
        // MapgenValues.heightmap = ImageIO.read(new File(MapgenValues.heightmapName));
        // // loadBMPImage("heightmap.bmp");
        // MapgenValues.stateBorderMap = ImageIO.read(new
        // File(MapgenValues.stateBordersName)); // loadBMPImage("state_borders.bmp");
        // } catch (IOException e) {
        // e.printStackTrace();
        // return;
        // }
        ...
        // MapgenValues.imageHeight = MapgenValues.heightmap.getHeight(); // may break
        // things but good idea
    ...
    /**
     * Generates seeds in a grid-like manner, but with a random offset which adds
     * some variance
     * while keeping provinces sizes/distances/density roughly even while still
     * looking
     * semi-random/natural.
     */
    private static void gridSeedGeneration() {
        ...
        // for(int y = MapgenValues.imageHeight / MapgenValues.numSeedsY / 2 - 1; y <
        // MapgenValues.imageHeight;
        // y += MapgenValues.imageHeight / MapgenValues.numSeedsY) {
        // for(int x = MapgenValues.imageWidth / MapgenValues.numSeedsX / 2 - 1; x <
        // MapgenValues.imageWidth;
        // x += MapgenValues.imageWidth / MapgenValues.numSeedsX) {
        // int xOffset = random.nextInt(MapgenValues.imageWidth / MapgenValues.numSeedsX
        // - 1)
        // - (MapgenValues.imageWidth / MapgenValues.numSeedsX / 2 - 1); // -3 to 3 //
        // should make variables // int xOffset = random.nextInt(numSeedsX - 1) -
        // (numSeedsX / 2 - 1);
        // int yOffset = random.nextInt(MapgenValues.imageHeight /
        // MapgenValues.numSeedsY - 1)
        // - (MapgenValues.imageHeight / MapgenValues.numSeedsY / 2 - 1); // -3 to 3 //
        // should make variables // int yOffset = random.nextInt(numSeedsY - 1) -
        // (numSeedsY / 2 - 1);
        // int seedX = x + xOffset; // x-value of seed
        // int seedY = y + yOffset; // y-value of seed
        //
        // /* heightmap color stuff */
        // // heightmap is in grayscale meaning only need to find red value to get
        // height value
        // // at point.
        // int heightmapHeight = (MapgenValues.heightmap.getRGB(seedX, seedY) >> 16) &
        // 0xFF;
        // int rgb = provinceColorGeneration(seedX, seedY, heightmapHeight); // rgb
        // color int value
    ...
    /**
     * Generates seeds dynamically, until there is a set number of seeds. Seed
     * density of sea can be set lower
     * than land.
     */
    private static void dynamicSeedGeneration() {
        // Random random = new Random(); // Random number generator for generating seed
        // locations
        // seeds = new ProvinceMapSeedsList();
        //
        // /*
